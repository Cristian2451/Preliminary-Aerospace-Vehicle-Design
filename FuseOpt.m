...
SigMax = SigY/2;
DsMin = 0.08; % minimum stringer separation to test
DsMax = 0.5;
% DsMin = 0.092727;
% DsMax = 0.092727;
...
if tMin < shearMax/SigShearY
    tMin = shearMax/SigShearY;
end


% determine values of three main parameters to test
sigma = linspace(SigMin,SigMax,n); % max stringer direct stress
Ds = linspace(DsMin,DsMax,n); % stringer separation
tSkin = linspace(tMin,tMax,n); % skin thickness
    ...
    for j = 1:n % tSkin
        for k = 1:n % Ds
            ...
            if sigCrit > sigma(i) % check critical buckling stress excedes sigma
                % stringers
                Nstring = round(2*pi*R/Ds(k)); % number of stringers
                thetaString = linspace(0,2*pi,Nstring+1); % angular distribution of stringers
                ...
                if Df > 0.35
                    ...
                    result(l,3) = Ds(k); % stringer spacing
                    result(l,4) = Hstring; % stringer height & width
                    result(l,5) = Df; % frame spacing
                    result(l,6) = Bframe; % frame base
                    result(l,7) = 2*pi*R*tSkin(j) + Nstring*Astring + 2*pi*R*Aframe/Df; % volume per length
                    l = l + 1;
...
disp(['Ds = ',num2str(result(minVolIndex,3))])
